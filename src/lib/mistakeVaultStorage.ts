...
export function addMistake(entry: {
  ...
  errorCategory?: CognitiveErrorCategory | string;
  category?: CognitiveErrorCategory | string;
  studentThoughtProcess?: string;
  notes?: string;
  tags?: string[];
  ...
  // Normalize category to valid CognitiveErrorCategory
  const rawCat = entry.errorCategory || entry.category || 'concept_gap';
  const validCategories: CognitiveErrorCategory[] = [
    'careless_calc',
    'misread_question',
    'formula_confusion',
    'concept_gap',
  ...
  let normalizedCategory: CognitiveErrorCategory = 'concept_gap';
  if (validCategories.includes(rawCat as CognitiveErrorCategory)) {
    normalizedCategory = rawCat as CognitiveErrorCategory;
  } else if (rawCat.includes('calc') || rawCat.includes('careless')) {
    normalizedCategory = 'careless_calc';
  } else if (rawCat.includes('read') || rawCat.includes('misread')) {
    normalizedCategory = 'misread_question';
  } else if (rawCat.includes('formula')) {
    normalizedCategory = 'formula_confusion';
  } else if (rawCat.includes('time') || rawCat.includes('rush')) {
    normalizedCategory = 'time_pressure';
  } else if (rawCat.includes('english') || rawCat.includes('language') || rawCat.includes('comprehension')) {
    normalizedCategory = 'english_comprehension';
  } else if (rawCat.includes('miss') || rawCat.includes('skip')) {
    normalizedCategory = 'missing_question';
  }

  const resolvedSubject = entry.subjectName || entry.subject || 'General';
  const resolvedTopic = entry.topicName || entry.topic || entry.chapter || 'Core Concepts';
  const resolvedAttempt = entry.userAttempt || entry.studentAnswer || '';
  const resolvedThought = entry.studentThoughtProcess || (entry.tags && entry.tags.length > 0 ? `Tags: ${entry.tags.join(', ')}` : undefined);
  ...
  const newMistake: MistakeEntry = {
    ...
    userAttempt: resolvedAttempt,
    correctAnswer: entry.correctAnswer,
    errorCategory: normalizedCategory,
    studentThoughtProcess: resolvedThought,
    notes: entry.notes || '',
